...
build-client:
	docker build --build-arg VITE_SERVER_URL=${VITE_SERVER_URL} --build-arg VITE_BOT_URL=${VITE_BOT_URL} --build-arg VITE_SERVER_STATIC_URL=${VITE_SERVER_STATIC_URL} -t ${DOCKER_LOGIN}/${DOCKER_CLIENT_NAME}:deploy -f ./apps/client/Dockerfile ./apps/client
build-server:
